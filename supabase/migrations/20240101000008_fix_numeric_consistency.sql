-- Ensure consistent numeric type usage
ALTER TABLE loops
ALTER COLUMN average_rating TYPE numeric(3,2);

ALTER TABLE profiles
ALTER COLUMN average_loop_rating TYPE numeric(3,2);

-- Drop and recreate search function with consistent numeric types
...
BEGIN
  ...
  SELECT 
    ...
    l.key,
    l.downloads,
    CAST(l.average_rating AS numeric(3,2)),
    l.created_at,
  ...
  ORDER BY
    CASE sort_by
      ...
      WHEN 'rating' THEN CAST(l.average_rating AS numeric(3,2))
      ELSE NULL
    END DESC NULLS LAST,
    CASE 
