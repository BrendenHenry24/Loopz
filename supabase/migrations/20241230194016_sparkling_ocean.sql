/*
  # Update Rating System Constraints

  1. Functions
    - Improve trigger function to prevent self-rating
    - Handle all rating update scenarios

  2. Security
    - Update RLS policies with proper constraints
    - Prevent users from rating their own loops via trigger
*/
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
    -- Get the producer_id for the loop being rated
    SELECT loops.producer_id INTO producer_id
    FROM loops 
    WHERE loops.id = NEW.loop_id;
